...
| Weapon panel: icon + name + `mag / reserve` + grenades + the other slot (top-center) | Local player loadout                  | Published the tick a **loadout signature** (weapon, mag, reserve, grenades, reloading, other slot) changes, plus the 10 Hz floor — so firing and reloading read live. The panel is a button: tapping it switches primary ↔ secondary (ADR-017) |
