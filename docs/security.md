...
| Compromised host device                   | The host browser _is_ the authority. A cheating **host** can do anything; the social fix (kick the host, re-host) is the only realistic one. Documented as residual risk (§8).                                             |
...
_Related: [architecture.md](architecture.md) (package boundaries), [networking.md](networking.md)
(transport, snapshot, reconciliation details), [testing.md](testing.md) (fuzz and validation
test suites), [performance.md](performance.md) (why caps are also perf guardrails)._
