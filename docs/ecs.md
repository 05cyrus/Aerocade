...
between archetypes) earns its complexity when entity _shapes_ vary at runtime and counts reach
tens of thousands. At our scale it adds indirection, allocation, and API surface while buying
nothing we need. What we _do_ need, it makes harder:
...
| Runs in browser _and_ Node with zero deps | Plain TS, no DOM/node types                               | Adds a runtime dependency to `shared`               |
...
- **Despawn** = `alive[i] = 0`. Nothing is cleared eagerly; the slot's data is garbage until the
  next spawn fully reinitializes it. Spawn functions must therefore write _every_ component
...
export function createPlayerPool(): PlayerPool {
  return {
    ...
    velY: new Float32Array(MAX_PLAYERS),
...
- **Float32 everywhere for continuous state.** Systems compute in JS doubles, but every value
  round-trips through a `Float32Array` each tick. Because the _stored_ representation is what
...
`projectiles` enqueued _this_ tick, and `respawn` must run after `damage` so a death and its
...
| --- | ------------- | ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `input`       | per-tick input commands (host: validated net inputs; local: device sampler)         | `world.inputs[*]`                                                                                        | `inputGateSystem`: the only system that rewrites inputs. Gates what the match phase permits — warmup clears the weapon buttons, a finished match clears everything (ADR-031) |
...
| 9   | `match`       | `Death`/`PickupTaken` events, `world.tick`, `world.match`                           | `world.match` (phase, clocks, winner, team frags), `MatchPhase` events                                   | Delegates scoring to a mode ruleset (ADR-031). FFA/TDM done; CTF/Survival slot in the same way                                                                               |

The four player-facing systems (`input`, `movement`, `physics`, `weapons`) also take an optional slot,
so a client can re-simulate **only its own player** during reconciliation through this exact code
rather than a second implementation of movement (ADR-033). Default is `ALL_PLAYERS`.
...
1. **Client prediction + reconciliation.** The client keeps a ring of predicted states keyed by
   tick. When a host snapshot acks input sequence _n_, the client restores its state buffer,
   overwrites it with the authoritative data, then **replays** inputs _n+1…now_ through the very
   same `stepSim`. Replay is trustworthy _only because_ restore is bitwise-exact and systems are
...
systems never re-enter and scratch is dead between calls (it is _not_ state: never snapshot it,
...
2. Register the field in the pool's **field manifest** — the single array-of-names each pool
   ...
   A field _not_ in the manifest is a bug the round-trip test catches.
   Scalar sim state that is not pool-shaped (currently only `world.match`) is exempt from the
   array manifest but obeys the same rule by hand: it has a `copyFrom` listing every field, it is
   mixed into `stateHash`, and a test mutates each field individually to prove the hash notices.
...
2. Insert it at an explicit position in the ordered system list in `stepSim` — never
   conditionally skipped on host vs client (branch _inside_ on world state if needed), or replay
...
- **Iteration order dependence done wrong.** Order-sensitive outcomes (contested pickup, damage
  application) are fine _only_ when resolved by ascending entity index (ADR-009). Never iterate
