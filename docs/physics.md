...
1. **X sweep.** Compute the target `x' = x + vx*dt`. Walk tile columns from the current
   ...
   *current* Y-span overlaps. On the first solid column, clamp the AABB flush to the tile
   face minus a skin of `SKIN = 0.001 m`, set `vx = 0`. Otherwise accept `x'`.
2. **Y sweep.** Same along Y using the *post-X-resolve* x position. Clamping while moving
...
3. **Grounded probe.** After resolution, `grounded = (vy >= 0)` AND a solid tile lies
   ...
   tick — no coyote time in the sim (any input-feel forgiveness is a client input concern,
   see [ui.md](ui.md)).
...
**Tunneling guarantee.** Because the sweep walks *every* tile boundary between old and new
positions, no per-axis speed can skip a wall. Player speed is additionally hard-capped at
`SPEED_CAP = 40 m/s` (post-impulse clamp, both axes combined) which keeps per-tick
...
| Jump cutoff factor | 0.45 | on release while rising: `vy *= 0.45` |
| Gravity | 21 m/s² | +y |
...
| Hover band | \|vy\| < 1.5 m/s | while thrusting |
| Hover burn | 20 fuel/s | ≈ 5 s hover on full tank |
| Speed cap | 40 m/s | post-impulse safety clamp |
...
No air friction: horizontal velocity persists, which is what makes rocket-jump arcs and
jetpack strafing feel right.

**Jump.** On jump press while grounded: `vy = −8.6`. **Variable cutoff:** if the jump
input is released while still rising (`vy < 0`), multiply `vy` by 0.45 once. Tap-jumps stay
short; held jumps reach full height. (Jetpack input is a separate held control — Space
doubles as both: press while grounded jumps, held while airborne thrusts.)
...
**Hover mode.** While thrust is held with fuel and `|vy| < 1.5`, thrust auto-modulates:
the applied acceleration becomes exactly `+gravity` cancellation plus a proportional pull
of `vy` toward 0 (critically damped within the band), holding altitude, and burn drops to
20 fuel/s. Leaving the band (e.g., knockback) restores full 38 m/s² thrust and 46/s burn
next tick. Hover is *emergent from held thrust* — there is no separate hover input.
...
- The `damage` system *queues* impulses; `movement` applies them at the start of the
...
- After applying impulses, total speed is clamped to `SPEED_CAP = 40 m/s` (§4).
- **Rocket jumps** fall out of the rules, they are not special-cased: fire the Thumper at
  the ground, take self-splash impulse (full strength) and self-splash *damage* scaled by
  `SELF_SPLASH_DAMAGE_SCALE = 0.5`. Survivable cost, big vertical Δv — deliberate movement
  tech per the weapon design intent.
- Knockback interacts with hover: a hit that pushes `|vy| ≥ 1.5` exits the hover band.
...
- **Lobber:** detonates on *first impact* — world tile or player — or when its 2.0 s fuse
  expires mid-flight, whichever comes first.
- **Frag grenade:** never impact-detonates; always the 2.6 s fuse. On tile contact it
  bounces: normal velocity component reflected and scaled by restitution **0.45**,
  tangential component scaled by `GRENADE_SURFACE_FRICTION = 0.82`. When post-bounce speed
  drops below `GRENADE_REST_SPEED = 0.8 m/s` on a supported tile, it rests until the fuse.
...
|---|---|---|---|
| Thumper rocket | 55 | 3.2 m | 13.5 m/s |
| Lobber grenade | 45 | 2.8 m | 10.0 m/s |
| Frag grenade | 62 | 3.4 m | 12.5 m/s |

Self-hits: full impulse, damage × 0.5 (§6). Explosions do not affect other projectiles or
...
- Cross-engine float divergence (trig/sqrt) is tolerated per ADR-009: the host is the
  single source of truth; we need practical determinism for replaying *our own* inputs,
...
| Jump cutoff | Tap jump 3 ticks vs. hold | Tap apex < hold apex; cutoff applies exactly once |
...
| Hover band entry/exit | Thrust with `vy = −1.4` / `−1.6` | Burn 20/s and altitude held inside band; 46/s outside |
| Hover displacement | Hover 120 ticks | `|Δy|` ≤ SKIN over the window |
| Knockback ordering | Impulse queued at tick N | Velocity changes at tick N+1, before integration |
| Speed cap | Stacked impulses | Post-clamp speed ≤ 40 m/s |
| Grenade restitution | Frag dropped from 3 m | Rebound `vy` = 0.45 × impact; rests below 0.8 m/s |
